...
public interface ClientRepository extends CrudRepository<Client, Long>, CritriaClientRepository {
    ...
    @Query(value = "select c.id, c.client_number, c.shortname, c.address, c.unp, " +
            ...
            " left join opday.officer o on o.id = c.officer_id " +
            " where c.status = 1 " +
            " and a1.analytic_code = :relationtype1 " +
            " and a2.analytic_code = :relationtype2 " +
            " and a3.analytic_code = :relationtype3 " +
            " and a4.analytic_code = :relationtype4 " +
            " order by c.id ", nativeQuery = true)
    List<Tuple> findAllByClientOpenDtoAnalyticFilter(@Param("relationtype1") Long relationtype1, @Param("relationtype2") Long relationtype2, @Param("relationtype3") Long relationtype3, @Param("relationtype4") Long relationtype4);
